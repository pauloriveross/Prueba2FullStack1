...
import duoc.mantenciones.exception.idMantencionNoEncontrada;
import duoc.mantenciones.exception.idVehiculoDuplicado;
...
public class MantencionesService {
    ...
    public Mantenciones guardarMantencion(MantencionesRequest request){
        if(mantencionesRepository.existsByIdVehiculo(request.getIdVehiculo())){
            throw new idVehiculoDuplicado("No se puede realizar dos mantenciones al mismo tiempo al mismo vehiculo");
        }
    ...
    public Mantenciones buscarPorId(Integer idMantencion){
        ...
        return mantencionesRepository.findById(idMantencion)
                .orElseThrow(() -> new idMantencionNoEncontrada("No se ha encontrado la mantencion con id: "+ idMantencion));
    }
    ...
    public Mantenciones actualizarMantencion(Integer idMantencion, MantencionesRequest request) {
        ...
        if (mantencionesRepository.existsByIdVehiculo(request.getIdVehiculo())
            ...
            throw new idVehiculoDuplicado("El vehiculo ya tiene una mantencion vigente");
        }
